...
module.exports = {
    getUserFiles: function(userId, callback) {
        ...
        User.findById(userId).exec(function(err, user) {
            ...
            FileContainer.find({ 'parent.id': user._id }).limit(10).exec(function(err, files) {
                if (err) {
                    console.error('Error fetching files for user:', err.message);
                    return callback(err);
                }

                console.log('Files retrieved:', files.length > 0 ? (files.length + ' files') : 'No files found');
                callback(null, files);
            });
        });
    },
            ...
            files.forEach(function(file) {
                ...
                self.parseCsvFile(filePath, function(err, csvData) {
                    ...
                    if (processedCount === files.length) {
                        console.log('Processed files sent to the frontend:', processedFiles);
                        res.render('gallery', {
                            user: req.user,
